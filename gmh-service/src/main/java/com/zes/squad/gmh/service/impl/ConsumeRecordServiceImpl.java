    ...
    private void calMoney(ConsumeRecordPo consumeRecord, List<ConsumeRecordDetailPo> consumeRecordDetails) {
        ...
        if (paymentWay == 1) {
            ...
            if (cmcPo.getIsValid() == 0) {
                throw new GmhException(ErrorCodeEnum.BUSINESS_EXCEPTION_OPERATION_NOT_ALLOWED, "此会员卡已失效");
            }
            if (cmcPo.getValidDate().before(new Date())) {
                throw new GmhException(ErrorCodeEnum.BUSINESS_EXCEPTION_OPERATION_NOT_ALLOWED, "此会员卡已过期");
            }
            ...
            List<CustomerMemberCardContentUnion> cardProjectLeftList = customerMemberCardContentMapper
                    .getProjectContentList(tmpCmcPo.getId());
            String moneyMessage = "";
            ...
            for (int i = 0; i < cardProjectLeftList.size(); i++) {
                projectMessage = projectMessage + cardProjectLeftList.get(i).getRelatedName()
