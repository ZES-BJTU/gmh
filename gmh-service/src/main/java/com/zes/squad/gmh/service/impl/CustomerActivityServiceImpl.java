...
public class CustomerActivityServiceImpl implements CustomerActivityService {
    ...
    public List<CustomerActivityUnion> getAcitvityListByMobile(Integer paymentWay, String customerMobile) {
        ...
        List<CustomerActivityUnion> customerActivityUnions = customerActivityMapper.getActivityListByCustomerId(map);
        if (paymentWay == 32) {
            for (CustomerActivityUnion cau : customerActivityUnions) {
                cau.setCustomerActivityContents(
                        customerActivityContentMapper.getCouponListByCustomerActivityId(cau.getId()));
            }
